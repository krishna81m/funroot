...
function createSession(quizId) {
  ...
  const session = {
    ...
    questionParticipants: new Set(), // players present when current question started
    createdAt: Date.now(),
  }
  sessions.set(pin, session)
  writeSnapshot(session)
  log.info('engine', `Session created pin=${pin} quiz=${quizId}`)
  return session
}
...
function addPlayer(pin, { socketId, identifier, nickname }) {
  ...
  // Notify all players of updated count (works in LOBBY and mid-game)
...
function skipCurrent(pin) {
  ...
  if (session.status === 'SLIDE') {
    ...
    broadcastStateSync(session)
    // Start the countdown timer — the auto-5s path in beginQuestion is now
    // bypassed, so the timer would never start without this explicit call.
...
function submitAnswer(pin, socketId, { questionId, answer, comment }) {
  const session = getSession(pin)
  ...
  history.push({ questionId, timeTaken, rawAnswer: answer, pointsEarned, isCorrect, comment: comment?.trim() || null })
  writeSnapshot(session)
  ...
  // Auto-advance when all players who were present at question start have answered.
  // Late-joining players do not count toward the threshold so they can't block advance.
  const participantCount = session.questionParticipants.size
  if (participantCount > 0 && answered >= participantCount) {
    TimerService.clear(session)
    transitionTo(session, 'RESULTS')
    broadcastResults(session)
  }
...
function beginQuestion(session, item) {
  ...
  // Snapshot who is present at question start — used for auto-advance and tally
  session.questionParticipants = new Set([...session.players.keys()])
...
function broadcastResults(session) {
  ...
  // Attribution: who answered what (sent after window closes — correct answers no longer secret)
  if (item) {
    const attributionAnswers = []
    for (const [socketId, logs] of session.playerHistory.entries()) {
      const log = logs.find((l) => l.questionId === item.id)
      if (!log) continue
      const player = session.players.get(socketId)
      if (!player) continue
      attributionAnswers.push({
    ...
    broadcast(session.pin, 'server:answer_attribution', {
      questionId: item.id,
      itemType: item.type,
...
async function finishGame(session) {
  ...
  log.info('engine', `pin=${session.pin} ${session.status} → FINISHED`)
